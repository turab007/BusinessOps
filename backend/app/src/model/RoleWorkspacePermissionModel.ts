...
import IRoleWorkspacePermission = require("./interfaces/IRoleWorkspacePermission");
 ...
 * 
 * @class RoleWorkspacePermission
 */
class RoleWorkspacePermission extends BaseModel<IRoleWorkspacePermission> {
     ...
     */
    createRoleWorkspacePermissions(role_id,workspace_id, permissionsAry) {

        // Read all role permissions
        return Promise.each(permissionsAry, (permissionObj) => {

            // Create Role Permissions Recursively
            return this.createRoleWorkspacePermissionRecursively(role_id, permissionObj.module_id, permissionObj.permission_id,workspace_id);
        })
    }
     ...
     */
    public createRoleWorkspacePermissionRecursively(role_id: string, module_id: string, permission_id: string,workspace_id:string) {

        // check if permission does not exist
        return this.findByAttribute({ role_id: role_id, permission_id: permission_id }).then(result => {

            if (!result) {
                let roleWorkspacePermission: IRoleWorkspacePermission = {
                    module_id: module_id,
                    role_id: role_id,
                ...
                return this.create(roleWorkspacePermission).then(rolePerResult => {
                    ...
                    return new PermissionChildrenModel().findAllByAttributes({ parent_id: permission_id }).then(children => {
                        ...
                        return Promise.each(children, (child) => {
                            ...
                            return this.createRoleWorkspacePermissionRecursively(role_id, module_id, child.child_id,workspace_id);
                        })
                    })
                });
            }
     ...
     * @param permissionsAry any[]
     */
    deleteRoleWorkspacePermissionsIfNotInAry(role_id: string, permissionsAry: any[]) {
        ...
        //             if (deletIt == true) {
        //                 return new RoleWorkspacePermission().delete(rolePermission._id.toString());
        //             }
    ...
    deleteRoleWorkspacePermission(roleId, permId,_wsId) {
        return this.deleteAllByAttributes({ role_id: roleId,permission_id:permId,workspace_id:_wsId });
     ...
     */
    checkRolesHavePermission(roleIds, permisison_id,wsID) {
        ...
        return new RoleWorkspacePermission().find(cond).then(rpResult => {
     ...
     * @param roleIds any[]
     */
    public getRolesWorkspacePermissions(roleIds: any[],ws:string) {

        console.log('===============role workspace permissions==============', roleIds);

        // Prepare condition
        let cond = { role_id: { $in: roleIds },workspace_id:ws }

        return this.find(cond);
    }

    public getPermissions(roleIds: any[],ws:string) {
        return new RoleWorkspacePermission().getRolesWorkspacePermissions(roleIds,ws).then(rolePermissions => {
...
Object.seal(RoleWorkspacePermission);
export { RoleWorkspacePermission };