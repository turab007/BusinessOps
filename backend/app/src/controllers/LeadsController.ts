...
import LeadModel = require("./../model/LeadModel");
import { StatusFlowModel,TagModel,TimeZoneModel,
    UserBusinessGroupModel,BusinessGroupModel } from "../model/barrel/"
...
class LeadsController extends BaseController {
    ...
    public create(req: express.Request, res: express.Response, next: express.NextFunction): void {
        ...
        try {
            ...
            let Lead: ILeadModel = <ILeadModel>req.body;


            let model = new LeadModel();
            model.createWithRelations(Lead, Lead.intersted_in).then((result) => {
                res.json(result);
            }).catch(next);
        }
        catch (error) {
    ...
    public update(req: express.Request, res: express.Response, next: express.NextFunction): void {
        try {
            ...
            var Lead: ILeadModel = <ILeadModel>req.body;
            ...
            model.updateWithRelations(_id, Lead, Lead.intersted_in).then((result) => {
    ...
    */
    ...
    /**
     * Get all the leads by Statues and status should be only specific to current business Group of 
     * current user
     * @param req 
     * @param res 
     * @param next 
     */
    public getLeadsGroupByStatusByCurrentBusinessGroup(req, res: express.Response, next: express.NextFunction): void {
        try {
            //TODO: Low (take this logic to model level)
            new UserModel().findByEmail(req.user.user_id).then(res1 => {
                console.log(res1._id);
                new UserBusinessGroupModel().getUserBuessinessIdsArray(res1._id).then(res2 => {
                    ...
                    new BusinessGroupModel().findByAttribute(cond).then((bgResult) => {
                        ...
                        new StatusFlowModel().findAllByAttributes(cond).then((stResult) => {
                            ...
                            new LeadModel().findAllByAttributes(cond).populate(['company_id','contact_id']).then(res1=>{
                                res.json({statuses: stResult, leads: _.groupBy(res1,'status')});
                            })
