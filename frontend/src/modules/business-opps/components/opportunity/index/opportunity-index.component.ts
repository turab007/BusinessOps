...
@Component({
    selector: 'opportunity-index-page',
    templateUrl: 'opportunity-index.component.html',
    styleUrls: ['opportunity-index.component.css']
})
export class OpportunityIndexComponent implements OnInit {
    ...
    actionButtons: any[] = [
        {
            'title': "View",
            'icon': "visibility",
            'url': "/business-opps/opportunities/view"
        },
        {
            'title': "Update",
            'icon': "edit",
            'url': "/business-opps/opportunities/update"
        },
        {
            'action': 'delete',
            'title': "Delete",
    ...
    private _loadData(): void {
        console.log('servies');
        this._opportunityService.index_paged(this.queryStrings).
            subscribe(
            response => {
     ...
     * delete record
     * @param obj
     *   will contains ({model: Opportunity, pageSize: 10, currentPage: 1}) 
     */
    deleteRecord(obj: Object) {
        //delete process
        let model: Opportunity = obj['model'];
        console.log(model);
