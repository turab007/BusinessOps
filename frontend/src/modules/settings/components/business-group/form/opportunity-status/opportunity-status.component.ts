...
import { OpportunityStatusDialogComponent } from './opportunity-status-dialog/opportunity-status-dialog.component';
...
export class OpportunityStatusComponent {
    ...
    private dialogRef: MatDialogRef<OpportunityStatusDialogComponent>;

    @Input() business_group_id: string;

    public opportunityStatuses: StatusFlow[];
    ...
    */
    ...
    private _loadData(): void {
        let queryString = {'filter': {'business_group_id':this.business_group_id,'type': 'Opportunity'}}; // will be used when needed
        this._statusFlowService.index_paged(queryString).subscribe(
            response => {
                this.opportunityStatuses = response['statuses'];
                console.log(this.opportunityStatuses);
            },
    ...
    openDialog(id?: string) {
        this.dialogRef = this.dialog.open(OpportunityStatusDialogComponent, {
            width: '600px',
            disableClose: false,
            data: {
